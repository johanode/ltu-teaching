...
För en icke-reparerbar enhet studeras livslängden $T$ och dess fördelning. Här är det naturligt att beskriva sannolikheten för att enheten fortfarande fungerar vid tiden $t$ med hjälp av funktionssannolikheten $R(t)$.
...
![Simulerad och teoretisk geometrisk fördelning](images/geometrisk-tid-till-fel.png)
...
En illustration av sambandet mellan täthetsfunktionen, fördelningsfunktionen och funktionssannolikheten ges i avsnittet [Exempel med normalfördelning](#exempel-med-normalfordelning).

I fortsättningen används därför konsekvent följande notation:

- $f(t)$ för täthetsfunktionen,
- $F(t)$ för fördelningsfunktionen,
- $R(t)$ för funktionssannolikheten, det vill säga överlevnadsfunktionen.
...
Diagrammen på sidan skapas av [`scripts/tid_till_fel.py`](scripts/tid_till_fel.py). Skriptet kan köras med

```bash
python scripts/tid_till_fel.py
```
